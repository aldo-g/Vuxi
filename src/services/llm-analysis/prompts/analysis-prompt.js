function getScoringDefinitions() {
  ...
  function createAnalysisPrompt(pageType, context, sections) {
    let prompt = `You are a UX/UI expert analyzing a ${pageType} for ${context.org_name || 'this organization'}, a ${context.org_type || 'non-profit'} organization.
      
      WEBSITE PURPOSE: ${context.org_purpose || 'to encourage donations and sign-ups for trainings'}
  ...
  function getAnalysisPrompt(type, data) {
    const context = {
      ...
      org_type: data.context?.org_type || 'non-profit',
      org_purpose: data.context?.org_purpose || 'to encourage donations and sign-ups for trainings'
    };
